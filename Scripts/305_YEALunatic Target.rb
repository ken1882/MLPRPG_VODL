...
class Game_Action
  ...
  def get_priority(role)
    if role.enemy?
      return 1000 if role.is_boss?
      return 100  if role.is_elite?
      return 10   if role.is_minon?
    else 
      return (10000 - role.hp) * 0.01
    end
  ...
  def lunatic_target(smooth_target)
    ...
    for effect in item.custom_targets
      ...
      #----------------------------------------------------------------------
      # Choose the one with least HP, priority effected by class level
      # <custom target: heal>
      #----------------------------------------------------------------------
      when /HEAL/i
        return default_target_set if default_target_set.size > 1
        
        candidates = []
        group = default_target_set[0].actor? ? $game_party.alive_members : $game_troop.alive_members
        
        for battler in group
          battler.priority = get_priority(battler)
          battler.priority *= (battler.mhp - battler.hp)
