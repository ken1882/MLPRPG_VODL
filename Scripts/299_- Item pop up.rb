...
class Game_Party < Game_Unit
  ...
  def gain_item(item, amount, include_equip = false, opp = Vocab::Coinbase,info = '')
    if !item_container(item.class).nil? && SceneManager.scene_is?(Scene_Map)
      if amount > 0
        info2 = sprintf("Party has gained item: %s (x%s)",item.name, amount)
        SceneManager.display_info(info2)
        RPG::SE.new(PearlItemPop::ItemSe, 80).play
      end
    end
    falcaopearl_itempop_gain(item, amount, include_equip, opp, info)
  end
  #-----------------------------------------------------------------------------
  # *) alias: gain_gold
  #-----------------------------------------------------------------------------
