...
class Game_Battler < Game_BattlerBase
  ...
  #--------------------------------------------------------------------------
  attr_accessor :effect_param_cache
  attr_reader   :effect_param_skill
  #--------------------------------------------------------------------------
  ...
  def initialize
    @effect_param_cache         = {}
    @effect_param_cache[:thac0] = nil
    @effect_param_cache[:ac]    = nil
    @effect_param_cache[:score] = Array.new(8){|i| nil}
    @effect_param_skill         = {}
    @effect_param_skill[:thac0] = [ nil, [] ]
    @effect_param_skill[:ac]    = [ nil, [] ]
    @effect_param_skill[:score] = Array.new(8, [ nil, [] ])
    init_battler_dnd
  end
  #--------------------------------------------------------------------------
  def get_skill_param_effect(symbol)
    value = @effect_param_skill[symbol].first
    return value if value
    return @effect_param_skill[symbol].first = calc_skill_param_effect(symbol)
  end
  #--------------------------------------------------------------------------
  def calc_skill_param_effect(symbol)
    value = (symbol == :score ? Array.new(8,0) : 0)
    @effect_param_skill[symbol].each do |skill|
      case symbol
      when :thac0; value += skill.thac0;
      when :ac;    value += skill.armor_class;
      when :score; 8.times{|i| value[i] += skill.param_adjust[i]};
      end
    end
    value
  end
  ...
  def attack_bonus_plus
    return @effect_param_cache[:thac0] if @effect_param_cache[:thac0]
    value = 0
  ...
  def armor_class_plus
    return @effect_param_cache[:ac] if @effect_param_cache[:ac]
    value = 0
    #------------------------------------------------
    # * process state effect
    #------------------------------------------------
