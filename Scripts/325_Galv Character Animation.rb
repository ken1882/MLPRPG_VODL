#NOTE THIS SCRIPT HAS BEEN MODIFIED BY HUTTY
...
#------------------------------------------------------------------------------#
  ...
  ANIM_SWITCH = 40             # ID of a switch to disable this effect.
                              # Turn switch ON in order to use change graphic
                              # move route commands. Turn off to restore anims.
                               
  DASH_SPEED = 0.9            # 1 is RMVX default dash speed.
...
class Sprite_Character < Sprite_Base
  ...
  def update
    ...
    if $imported["CXJ-FreeMovement"]
      return move_anim if $game_player.move_poll.size > 0
    end
    
    @idletime += 1
    idle_anim if @idletime == 100
