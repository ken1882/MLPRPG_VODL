  ...
  def load_notetags_specdef
    ...
    self.note.split(/[\r\n]+/).each { |line|
      ...
      when MOTO::REGEXP::BASEITEM::ARMOR_CLASS
        $data_notetagged_items.push(self)
        @armor_class = $1.to_i
        p sprintf("[Attack Block]:%s AC adjust: %d",self.name,self.armor_class)
...
class Game_BattlerBase
  ...
  def did_block?(item, is_crit = false,user)
    ...
    rate = 0
    result = false
    real_ac = self.armor_class
    
    @states.each do |state|
      real_ac += $data_states[state].armor_class
    end
    ...
    if actor?
      for equip in equips
        ...
        result = true if (real_ac + adjusted_block_rate(equip, item)) > @base_attack_roll
      end
    end
