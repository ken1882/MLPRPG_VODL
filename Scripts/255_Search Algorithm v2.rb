...
class PathFinding_Queue
  ...
  def predict_movement_cost(source_location,current_location,goal_location)
    ...
    return (dx1.abs + dy1.abs) * 3 + (dx1 * dy2 - dx2 * dy1).abs * 3
  end
...
class Game_Character < Game_CharacterBase
  ...
  def move_to_position(goalx, goaly, args = {})
    ...
    #puts "[Pathfinding] Source: #{@x} #{@y}"
    #puts "[Pathfinding] Goal  : #{ori_goalx} #{ori_goaly}"
    #puts "[Pathfinding] offset: #{offset_x} #{offset_y}"
    tox   = [0,-1,1,0]
    toy   = [1,0,0,-1]
    dir   = [2,4,6,8]
    cnt   = 0
      ...
      for i in 0...4
        ...
        if ( $game_map.passable?(next_x,next_y,dir[i]) || adjacent?(next_x, next_y, goalx, goaly) ) && 
          ...
          if adjacent?(next_x, next_y, goalx, goaly)
            path_found = true
          elsif (((next_x - goalx).abs + (next_y - goaly).abs ) <= tool_range - 1 ||
    ...
    if path_found
      ...
      @pathfinding_moves = @pathfinding_moves + path_queue.get_walk_path(goalx,goaly)
    end # if path found
