...
@element_window.back_opacity = 0
  ...
  def make_command_list
    ...
    @actor.calc_score_ability_skill
...
@element_window.back_opacity = 0
  ...
  def draw_property(property, dx, dy, dw)
    ...
    when :p_str
    value = sprintf(dmt, @actor.real_str)    
    when :p_dex
    value = sprintf(dmt, @actor.real_dex)    
    when :p_con
    value = sprintf(dmt, @actor.real_con)    
    when :p_int
    value = sprintf(dmt, @actor.real_int)    
    when :p_wis
    value = sprintf(dmt, @actor.real_wis)   
    when :p_cha
    value = sprintf(dmt, @actor.real_cha)
    when :jp
    value = sprintf(dmt, @actor.jp(@actor.class_id))
    when :crafting_level
    value = sprintf(dmt, @actor.crafting_level)
    when :armor_class
      value = sprintf(dmt, @actor.armor_class)
    when :thac0
      value = sprintf(dmt, @actor.thac0)
    when :ppn
      value = sprintf(dmt, @actor.p_penetration)
    when :mpn
      value = sprintf(dmt, @actor.m_penetration)
    when :saving_throw01
      value = sprintf(dmt, @actor.saving_throw_adjust[1])
    when :saving_throw02
      value = sprintf(dmt, @actor.saving_throw_adjust[2])
    when :saving_throw03
      value = sprintf(dmt, @actor.saving_throw_adjust[3])
    when :saving_throw04
      value = sprintf(dmt, @actor.saving_throw_adjust[4])
    when :saving_throw05
      value = sprintf(dmt, @actor.saving_throw_adjust[5])
    when :saving_throw06
      value = sprintf(dmt, @actor.saving_throw_adjust[6])
    when :str_ath
      value = sprintf(dmt, @actor.str_ath)
    when :dex_acr
      value = sprintf(dmt, @actor.dex_acr)
    when :dex_sle
      value = sprintf(dmt, @actor.dex_sle)
    when :dex_ste
      value = sprintf(dmt, @actor.dex_ste)
    when :int_arc
      value = sprintf(dmt, @actor.int_arc)
    when :int_his
      value = sprintf(dmt, @actor.int_his)
    when :int_inv
      value = sprintf(dmt, @actor.int_inv)
    when :int_nat
      value = sprintf(dmt, @actor.int_nat)
    when :int_rel
      value = sprintf(dmt, @actor.int_rel)
      
    when :wis_ani
      value = sprintf(dmt, @actor.wis_ani)
    when :wis_ins
      value = sprintf(dmt, @actor.wis_ins)
    when :wis_med
      value = sprintf(dmt, @actor.wis_med)
    when :wis_per
      value = sprintf(dmt, @actor.wis_per)
    when :wis_sur
      value = sprintf(dmt, @actor.wis_sur)
      
    when :cha_dec
      value = sprintf(dmt, @actor.cha_dec)
    when :cha_int
      value = sprintf(dmt, @actor.cha_int)
    when :cha_perfor
      value = sprintf(dmt, @actor.cha_perfor)
    when :cha_persua
      value = sprintf(dmt, @actor.cha_persua)
