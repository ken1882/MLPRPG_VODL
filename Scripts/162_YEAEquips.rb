...
module YEA
  module EQUIP
    ...
    DEFAULT_BASE_SLOTS = [ 0,1,1,2,3,5, 6, 7, 4, 4]
...
class Window_EquipStatus < Window_Base
    ...
    if !@temp_item.nil?
      for id in 0..8 do
        ...
        when 0
          ...
          cnt += 1
        when 1
          #draw_dndparam_bonus(0,line_height * cnt)
        ...
        when 2
          if @temp_item.is_a?(RPG::Weapon)
            draw_weapon_speed(0,line_height * cnt) 
            cnt += 1
          end
        when 3
          if @temp_item.is_a?(RPG::Weapon)
            draw_weapon_range(0,line_height * cnt)
            cnt += 1
          end
        when 4
          if @temp_item.is_a?(RPG::Weapon)
  ...
  def get_weapon_type_name(id)
    wtype_name = ["","Hoof Axe","Horseshoe","Polearm","1H Sword","2H Sword",
    "Bow","Crossbow","Hammer","Mace","Firearm","Arrow","Bolt","Bullet"]
    return wtype_name[id]
  end
  ...
  def draw_actor_attack_bonus(x,y)
    ...
    draw_text(0, y, w, line_height,"Attack Bonus")
  ...
  def draw_actor_armor_class(x,y)
    w = contents.width
    change_color(system_color)
    draw_text(0, y, w, line_height,"Armor Class")
    change_color(normal_color)
  ...
  def draw_item_type(x,y)
    w = contents.width
    change_color(system_color)
    draw_text(0, y, w, line_height,"Type")
    change_color(normal_color)
    if @temp_item.is_a?(RPG::Weapon)
  ...
  def draw_dndparam_bonus(x,y)
    ...
    change_color(system_color)
    if @temp_item.is_a?(RPG::Weapon)
      draw_text(0, y, w, line_height,"Attack")
      change_color(normal_color)
      draw_text(0, y, w, line_height,@temp_item.attack_bonus, 2)
      
    elsif @temp_item.is_a?(RPG::Armor)
      draw_text(0, y, w, line_height,"AC")
      change_color(normal_color)
  ...
  #--------------------------------------------------------------------------
  def draw_dndparam_compare(x,y,type)
    ...
    draw_current_dndparam(x + 4, y, type) if @actor
    drx = (contents.width + 22) / 2
    draw_right_arrow(drx, y)
    draw_new_dndparam(drx + 22, y, type) if @temp_actor
    reset_font_settings
  end
  ...
  #--------------------------------------------------------------------------
  def draw_current_dndparam(dx, dy, type)
    change_color(normal_color)
  ...
  def draw_new_dndparam(dx, dy, type)
    ...
    if type == 0
      ...
      draw_text(0, dy, contents.width-4, line_height, @temp_actor.attack_bonus, 2)
    elsif type == 1
      new_value = @temp_actor.armor_class
      change_color(param_change_color(new_value - @actor.armor_class))
      draw_text(0, dy, contents.width-4, line_height, @temp_actor.armor_class, 2)
    end
  ...
  def draw_weapon_speed(x,y)
    w = contents.width
    change_color(system_color)
    draw_text(0, y, w, line_height, "Cooldown(1/60 sec.)")
    change_color(normal_color)
    
    speed = @temp_item.tool_speed
  ...
  def draw_weapon_range(x,y)
    w = contents.width
    change_color(system_color)
    draw_text(0, y, w, line_height, YES::SKILL_EQUIP::PROPERTIES[:range])
    change_color(normal_color)
    draw_text(0, y, w, line_height, 0, 2)
  end
  #--------------------------------------------------------------------------
  # draw_damge_index
  #--------------------------------------------------------------------------
  def draw_weapon_damage(x,y)
    w = contents.width
    change_color(system_color)
    draw_text(x, y, w, line_height, YES::SKILL_EQUIP::PROPERTIES[:damage])
    change_color(normal_color)
    cnt = 1
    for feat in @temp_item.damage_index
      info = sprintf("%dd%d +(%d), %s; %s",feat[0],feat[1],feat[2],$data_system.elements[feat[3]],get_param_name(feat[4]))
