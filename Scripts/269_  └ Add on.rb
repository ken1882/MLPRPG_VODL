...
class Scene_Base
  ...
  def update_basic
    ...
    update_debug_window if debug_mode?
  end
  #--------------------------------------------------------------------------
  def update_mutex
  ...
  def create_debug_window
    @debug_window = Window_Base.new(0, 0, Graphics.width, Graphics.height)
    @debug_window.z = 5000
    @debug_window.deactivate
    @debug_window.hide
  end
  #----------------------------------------------------------------------------
  ...
  def update_debug_window
    ...
    return unless Input.trigger?(:kF10)
    heatup_button
