...
class Game_Party < Game_Unit
  ...
  def gain_item(item, amount, include_equip = false, play_se = true)
    if !item_container(item.class).nil? && SceneManager.scene_is?(Scene_Map)
      if amount > 0
        ...
        SceneManager.display_info(info) if play_se
        RPG::SE.new(PearlItemPop::ItemSe, 80).play if play_se
      end
    end
    falcaopearl_itempop_gain(item, amount, include_equip)
  end
  #-----------------------------------------------------------------------------
  # *) alias: gain_gold
  #-----------------------------------------------------------------------------
