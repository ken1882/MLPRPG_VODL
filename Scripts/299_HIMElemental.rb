...
class Game_Battler < Game_BattlerBase
  ...
  def item_element_rate(user, item)
    if item.damage.element_id < 0
      ...
      v = 1
      #p sprintf("item element id :%s %d",item.name,item.element_id)
      for i in user.states
         i.load_notetag_atk_element_modifiers
         #p sprintf("state element id :%s %d",i.name,i.element_id)
         
         if !i.atk_element_modifiers.empty? && i.element_id == item.element_id
           if v == 1
             v = item_multi_element_rate(user, i)
             #p sprintf("Init mutipler %d",v)
           else
             v *= (1+(item_multi_element_rate(user, i)/20)).to_f
           end
         elsif item.element_id - 1 <= 0  || (11 <= item.element_id && item.element_id <= 13)
           v *= (1+(item_multi_element_rate(user, i)/25)).to_f
         end
         
      end
      
      #p sprintf("element mutipler : %f",v)
      if item.atk_element_modifiers.empty?
        return th_attack_atk_element_modifiers_item_element_rate(user, item) * v
      else
        return item_multi_element_rate(user, item) * v
      end
    end
  end
