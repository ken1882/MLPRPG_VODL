...
class Game_Battler < Game_BattlerBase
  ...
  def roll_base_damages(user, item)
    if item.is_a?(RPG::Weapon) && user.is_a?(Game_Actor)
      ammo_slot_id = user.class.ammo_slot_id
      if item.ammo_type_id != nil && user.equips[ammo_slot_id] != nil
        item = user.equips[ammo_slot_id]
      end
    end
...
class Game_Battler < Game_BattlerBase
  ...
  def make_base_result(user, item, source_item)
    ...
    a = source_item.is_a?(RPG::Weapon) ? user.attackDC(item,value) : user.skillDC(source_item)
    b = self.armor_class
