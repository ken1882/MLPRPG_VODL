...
#The names of the categories for All Crafting Menu, use :All for all recipes
  CATEGORIES = [:Alchemy,:Cook,:Blacksmith,:Armorer,:Etceterer]
  #:Carpenter,:Builder,:Tailor
  DESCRIPTIONS = {
    :Alchemy => "The science of nature! Allow you to mix this and that...\nPotion! (Require role:Twilight Sparkle)",
    :Cook => "All you have to do is take a cup of flour! Add it to the mix!\n (Require role:Pinkie Pie)",
    :Blacksmith => "You can smithing you own weapon! (Require role:Applejack)",
    :Armorer => "Forge your custom armor, give your pony more bouns in armoring!\n (Require role:Applejack)",
    #:Tailor => "The ability to sew numerous things together to make another thing.",
    #:Carpenter => "Everything's made out of wood."
    :Etceterer => "This skill allow you to extract/smelt normals to advanced materials!\n (Require role:Rarity)"
  }
#The icons for categories, same order as above
  CATEGORY_ICONS = [8980,553,9004,8243,9106]
#The icon for player level
  LEVEL_ICON = 117
#Xp needed for crafting level, lvl is used for current level
  XP_NEEDED_EQUATION = "100 * lvl + Math.sqrt(lvl*10).to_i * 15"
...
# :category => :category        - Crafting category (:Cook, :Tailor, etc)
...
#  xp gain = xp - (current_level - recipe_level) * xp_deprac
  ...
  RECIPES = {
  #basic ingredients
  0 => { :result => [0,102,1], #flour = 1*water + 1*wheat
         :materials => [[0,100,1],[0,101,1]],
         :gold_cost => 10,
         :success =>100,
         :success_gain => 1,
         :level => 5,
         ...
         :xp => 5,
         :xp_deprac => 0,},
  1 => { :result => [0,2,1], #apple pie = 1*apple + 1*flour
         :materials => [[0,1,1],[0,102,1]],
         :gold_cost => 15,
         :success =>90,
         :success_gain => 1,
         :level => 5,
    ...
    2 => { :result => [0,114,1], #Steel = 1*iron ore + 1*coal
         :materials => [[0,113,1],[0,112,1]],
         :gold_cost => 20,
         :success =>85,
         :success_gain => 1,
         :level => 5,
   ...
   3 => { :result => [0,116,1], #gold = 1*gold ore + 1*coal
         :materials => [[0,115,1],[0,112,1]],
         :gold_cost => 100,
         :success =>80,
         :success_gain => 1,
         :level => 10,
   ...
   4 => { :result => [0,119,1], #Dordlan = 2*black sap + 5*sand + 3*sour liquid
         :materials => [[0,122,2],[0,120,5],[0,121,3]],
         :gold_cost => 200,
         :success =>70,
         :success_gain => 1,
         :level => 20,
...
class Recipe
  ...
  def has_level?
    case @category
      when :Alchemy
        return $game_party.members.include?($game_actors[1]) && @level <= $game_actors[1].level && has_craft_level? 
      when :Cook
        return $game_party.members.include?($game_actors[4]) && @level <= $game_actors[4].level && has_craft_level? 
      when :Armorer
        return $game_party.members.include?($game_actors[2]) && @level <= $game_actors[2].level && has_craft_level? 
      when :Blacksmith
        return $game_party.members.include?($game_actors[2]) && @level <= $game_actors[2].level && has_craft_level? 
      when :Etceterer
        return $game_party.members.include?($game_actors[6]) && @level <= $game_actors[6].level && has_craft_level? 
      else
        return has_craft_level? && @level <= $game_party.highest_level
    end
  end
...
class Game_Party
  ...
  def gain_craft_exp(id, val)
    ...
    while craft_exp(id) >= craft_exp_next(id)
      ...
      @crafting_level[id] += 1
      #================================
      #   Pony's crafting level
      #================================
      case id
      when 0 # Alchemy
        $game_actors[1].crafting_level = @crafting_level[id]
      when 1 # Cooking
        $game_actors[4].crafting_level = @crafting_level[id]
      when 4 # Etceterer
        $game_actors[6].crafting_level = @crafting_level[id]
      else
        $game_actors[2].crafting_level = @crafting_level[id]
      end
    end
  end
end
...
class Window_RecipeDetail < Window_Base
  ...
  def set_recipe(recipe)
    @recipe = recipe
    refresh unless $last_recipe == recipe rescue nil
    $last_recipe = recipe
  end
  ...
  def draw_craft_level
    ...
    draw_text(0,0,contents.width,contents.font.size,"Require Craft/Role Lv. :")
    ...
    if !ADV_RECIPE::DISABLE_PLAYER_LEVEL
      ...
      pony_role_icon = ADV_RECIPE::LEVEL_ICON
      ...
      draw_icon(pony_role_icon,contents.width - 48,0)
      xx += 48
...
class Window_RecPopup < Window_Selectable
  ...
  def set_text(item, amount)
    ...
    Audio.se_play('Audio/SE/Bell2',80,100)
