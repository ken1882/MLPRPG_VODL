  ...
  def update
    ...
    if @mouse_exist
      for target in @event_window.participants
        if Mouse.adjacent?(target.x, target.y)
...
class Scene_QuickTool < Scene_MenuBase
  ...
  def open_slots
    ...
    puts "#{@slot_confirm.current_symbol}"
    if @type_select.current_symbol == :item
      case @slot_confirm.current_symbol
      when :slot1 then actor.assigned_item[0] = @items_w.item
      when :slot2 then actor.assigned_item[1] = @items_w.item
      when :slot3 then actor.assigned_item[2] = @items_w.item
      end
    else
      case @slot_confirm.current_symbol
      when :slot1  then actor.assigned_skill[0] = @items_w.item
      when :slot2  then actor.assigned_skill[1] = @items_w.item
      when :slot3  then actor.assigned_skill[2] = @items_w.item
      when :slot4  then actor.assigned_skill[3] = @items_w.item
      when :slot5  then actor.assigned_skill[4] = @items_w.item
      when :slot6  then actor.assigned_skill[5] = @items_w.item
      when :slot7  then actor.assigned_skill[6] = @items_w.item
      when :slot8  then actor.assigned_skill[7] = @items_w.item
      when :slot9  then actor.assigned_skill[8] = @items_w.item
      when :slot10 then actor.assigned_skill[9] = @items_w.item
      end
    end
