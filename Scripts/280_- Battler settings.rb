...
class Game_Party < Game_Unit
  ...
  # tag: skill
  #---------------------------------------------------------------------------
  def set_skill(actor, skill, slot)
    actor = $game_actors[actor] if actor.is_a?(Fixnum)
    skill = $data_skills[skill] if skill.is_a?(Fixnum)
    return unless actor.skill_learn?(skill)
    actor.assigned_skill[slot] = skill
  end
  
  def set_item(actor, item, slot)
    actor = $game_actors[actor] if actor.is_a?(Fixnum)
    item  = $data_items[item] if skill.is_a?(Fixnum)
    return unless has_item?(item)
    actor.assigned_item[slot] = item
  end
end
#--------------------------------------------------------
...
class Game_Actor < Game_Battler
  ...
  attr_accessor :primary_use
  attr_accessor :usability
  attr_accessor :assigned_sskill
  ...
  def setup(actor_id)
    ...
    @usability = []
    for i in 0...(@assigned_item.size + @assigned_skill.size + 3)
      @usability[i] = nil
    end
  ...
  def apply_usability
    ...
    cnt = 2
    @assigned_item.each_key do |key|
      @usability[cnt] = usable?( @assigned_item[key] ) if !@assigned_item[key].nil?
      cnt += 1
    end
    
    @assigned_skill.each_key do |key|
      @usability[cnt] = usable?( @assigned_skill[key] ) if !@assigned_skill[key].nil?
      cnt += 1
    end
    
    @usability[cnt] = usable?(@assigned_sskill) if !@assigned_sskill.nil?
  end
 #-------------------------------------------------------------------------------
