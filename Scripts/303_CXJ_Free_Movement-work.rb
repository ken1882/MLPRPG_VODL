...
class Game_CharacterBase
  ...
  # # tag: movement
  #--------------------------------------------------------------------------
  def interpret_move(step_left = nil)
    return if @move_poll.empty?
    step_left = distance_per_frame if step_left.nil?
  ...
  def pixel_passable?(px,py,d)
    return true if self.is_a?(Game_Projectile) && projectile_to_lower?(px, py, d)
  ...
  def collide_with_events?(x, y, ignore_ally)
    $game_map.events_xy_rect_nt(x, y, collision_rect).any? do |event|
      next if ignore_ally && event.enemy.nil? && (!event.event.name.include?("<block>") && self.is_a?(Game_Projectile))
  ...
  def force_blocked(x, y, ignore_ally)
    $game_map.events_xy_rect_nt(x, y, collision_rect).any? do |event|
      event.event.name.include?("<block>") && self.is_a?(Game_Projectile)
    end
  end
  #--------------------------------------------------------------------------
...
class Game_Character < Game_CharacterBase
  ...
  def move_toward_character(character, pathfinding = false, tool_range = 1, move_fixed = true)
    return unless @move_poll.empty?
    
    if pathfinding && distance_to(character.x, character.y) > 3 &&
      @pathfinding_moves.empty? && @move_poll.empty?
      @pathfinding_goal = character
      found = move_to_position(character.x, character.y, tool_range: tool_range) unless self.command_holding?
      clear_pathfinding_moves if @pathfinding_goal.is_a?(Game_Event) && @pathfinding_goal.enemy.dead?
      return if found
    end
    ...
    processed = false
    fixed_move = (32.0 / CXJ::FREE_MOVEMENT::PIXELS_PER_STEP).ceil - 1
    if sx.abs > sy.abs
      if passable?(@x, @y, (sx > 0 ? 4 : 6))
        @move_poll+= [[sx > 0 ? 4 : 6, true]]
        @move_poll+= [[sx > 0 ? 4 : 6, true]] * fixed_move if move_fixed
      else
        @move_poll+= [[sy > 0 ? 8 : 2, true]]
        @move_poll+= [[sy > 0 ? 8 : 2, true]] * fixed_move if move_fixed
      end
      processed = true
    elsif sy != 0
  ...
  def start_map_event(x, y, triggers, normal, rect = collision_rect)
    ...
    $game_map.events_xy_rect(x, y, rect).each do |event|
      if event.trigger_in?(triggers) && event.normal_priority? == normal && (!$game_party.leader.state?(2) || event.event.name.include?("<forced>"))
        ...
        next if Mouse.click?(1) && event.distance_to(mx, my) > 1
