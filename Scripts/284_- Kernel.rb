...
class Game_CharacterBase
  ...
  attr_accessor :ch_oy # tag: modified
  #----------------------------------------------------------------------------
  #----------------------------------------------------------------------------
  # *) Initialize Object
  #----------------------------------------------------------------------------
  ...
  #def battler
  #  return $game_party.leader if self.is_a?(Game_Player)
  #  return self.actor if self.is_a?(Game_Follower)
  #  return @enemy if self.is_a?(Game_Event)
  #end
  ...
  def use_skill(id)
    return unless tool_can_use?
    skill = id.is_a?(RPG::Skill) ? id : $data_skills[id]
    process_tool_action(skill)
  end
  ...
  def usable_test_passed?(item)
    ...
    itemcost = item.tool_itemcost
    
    if battler.is_a?(Game_Actor) and (item.tool_itemcost != nil || item.tool_itemcosttype != nil)
      
      if (item.tool_itemcost != 0 && !battler.usable?($data_items[itemcost])) ||
        (item.tool_itemcosttype != nil && !battler.weapon_ammo_ready?(item))
        if @hint_cooldown == 0
  ...
  def load_abs_tool(item)
    ...
    if PearlKernel.has_data?
      consume_ammo_item(item) if battler.is_a?(Game_Actor) && item.item_required?
  ...
  def create_projectile_object(item)
    if PearlKernel.tool_special == "hook"
      ...
      $game_player.projectiles.push(Projectile.new(self, item, nil, true))
    end
  end
  ...
  def consume_ammo_item(item)
    itemcost = item.tool_itemcost == 0 ? item.tool_itemcosttype : $data_items[item.tool_itemcost]
    return if item.is_a?(RPG::Item) and item.consumable and item == itemcost
    
    slot_id = battler.class.ammo_slot_id
    item = battler.equips[slot_id]
  ...
  def update_state_effects
    return if battler.nil?
    battler.reg_time_count += 1 unless (battler.hrg == 0.0 && battler.mrg == 0.0)
    ...
    update_regenerate if battler.reg_time_count >= 15
  end
  #---------------------------------------------------------------------------
  # *) update_regenerate
  #---------------------------------------------------------------------------
  def update_regenerate
    battler.reg_time_count = 0
...
class Game_Event < Game_Character
  ...
  def register_enemy(event)
    ...
    if !$game_system.remain_killed[$game_map.map_id].nil? && 
