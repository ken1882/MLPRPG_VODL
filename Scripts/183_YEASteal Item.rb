...
class Game_Battler < Game_BattlerBase
  ...
  def execute_steal_effect(user, item)
    ...
    if stealable_items == [] && $game_troop.available_items.size < 1
      user.dmg_popup = true 
      if user.id == 6
        Audio.se_play("Audio/SE/RR_It's_Nothing_to_sneeze_at",100,100)
      else
        Audio.se_play("Audio/SE/Miss",100,100)
      end
      user.popup_ary.push((["Nothing","Nothing"]))
      return
    end
  ...
  def apply_steal_effect(user, item)
    ...
    if !@result.success
      for item in $game_troop.available_items
        next if item[0] == 0 || item.nil?
        if rand() < 0.4
          stealable_item = RPG::Enemy::StealItem.new(1,item[0], 0.4)
          #puts "Item stolen: #{stealable_item}"
          @result.stolen_item = stealable_item
          $game_troop.reduce_item(item[0])
          $stolen_from_enemy_bag = true
          @result.success = true
          break
        else 
          #puts "#{item[0]} failed"
    ...
    if !@result.success
      user.dmg_popup = true
      $stolen_from_enemy_bag = false
      Audio.se_play("Audio/SE/Miss",100,100)
      user.popup_ary.push((["Fail","Fail"]))
    end
  end
...
class Scene_Battle < Scene_Base
  ...
  def apply_steal_results(target, item)
    ...
    if target.stealable_items.empty? && !$stolen_from_enemy_bag
      fmt = YEA::STEAL::STEAL_EMPTY_TEXT
      text = sprintf(fmt, target.name)
    elsif target.result.stolen_item.nil? && !$stolen_from_enemy_bag
    ...
    #1.times do @log_window.wait end
    @log_window.back_one
  end
  ...
  def stolen_item_text(target)
    ...
    target.remove_stealable_item(stolen_item) unless $stolen_from_enemy_bag
    ...
    when 4 # Gold
      ...
      target.damage.push([value.to_i,"Gold"])
      return sprintf(fmt, Icon.steal_gold, value, Vocab.currency_unit)
    end
    
    $game_party.gain_item(item, 1)
    $game_party.execute_item_popup(item)
