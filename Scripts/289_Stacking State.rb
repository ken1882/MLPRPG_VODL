...
class Game_Battler < Game_BattlerBase
  ...
  def add_state(state_id)
    ...
    if state_addable?(state_id)  ## Line below changed to allow stacking states.
      cur_stack_level = state_stack(state_id)
      ...
      if !max_state_stack?(state_id)
        adjust_id = state_id + cur_stack_level
        
        
        tsbs_add_state(state_id) if state_id == 48
        ...
        if adjust_id != state_id
          remove_state(adjust_id - 1) if adjust_id-1 != state_id
          add_new_state(adjust_id)
          @result.added_states.push(adjust_id).uniq!
          reset_state_counts(adjust_id)
        end
      end
      
    end
    #p sprintf("%d added? : %s",state_id,state?(state_id))
  end
  
  def update_state_turns
    states.uniq.each do |state|
      next if @state_turns.nil? || @state_turns[state.id].nil?
