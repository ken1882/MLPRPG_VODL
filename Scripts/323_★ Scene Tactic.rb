...
class Scene_Map < Scene_Base
  ...
  # * Start Processing
  #--------------------------------------------------------------------------
  alias start_scenemap_opt start
  def start
  ...
  def update
    super
    return if $on_exit || @@overlayed
    
    update_tactic_buttons
    @spriteset.update
    $game_map.update(true) if $game_map.need_refresh
    if @tactic_enabled && @stack.last != InfoWindowEnabled
      $tbs_cursor.update if @stack.last != BattlerSelected
      update_scene       if !Input.press?(:kALT) && $tbs_cursor.mode.nil?
      update_win_visible if !$game_message.busy?
    end
    
    return if @tactic_enabled && !Input.press?(:kALT) || $game_message.busy?
  ...
  def update_win_visible
    ...
    @windows[Win_Status].visible = @tactic_enabled && @selected && !$game_message.busy?
...
class Spriteset_Map
  ...
  #--------------------------------------------------------------------
  def update_cursor
    return if @cursor.nil? || !$tactic_enabled
    @cursor.update
    @units.each do |unit|
      unit.update
    end
...
class Game_Map
  ...
  def occupied_by?(x, y, sx = 0 ,sy = 0)
    ...
    for sprite in $battle_unit_sprites
      ...
      _battler = $game_party.leader if battler.is_a?(Game_Player)
      _battler = battler.actor      if battler.is_a?(Game_Follower)
      _battler = battler.enemy      if battler.is_a?(Game_Event)
