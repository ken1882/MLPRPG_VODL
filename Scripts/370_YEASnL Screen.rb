...
module YEA
  module SAVE
    ...
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    MAX_FILES = 54         # Maximum saves a player can make. Default: 16
...
class Window_FileAction < Window_HorzCommand
  ...
  def call_ok_handler
    ...
    elsif current_symbol == :save && DataManager.save_file_exists?(@file_window.index + 1)
      info = "   Selected file will be over-  written, continue?"
      raise_overlay(info , :call_handler, current_symbol)
    elsif current_symbol == :delete
      info = "Do you really wanna delete this file?"
      raise_overlay(info , :call_handler, current_symbol)
    else
      super
    end
  ...
  def save_enabled?
    ...
    return false if BattleManager.in_battle? || $game_switches[9]
    return true
  end
