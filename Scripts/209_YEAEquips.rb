...
class Window_EquipStatus < Window_Base
  ...
  def draw_hoof_damage(x,y)
    w = contents.width
    change_color(system_color)
    draw_text(x, y, w, line_height, "Weapon Damages")
    change_color(normal_color)
    cnt = 1
    ...
    if @actor.equips[0].nil?
      ...
      for feat in @actor.equips[0].damage_index
  ...
  def draw_current_dndparam(dx, dy, type)
    ...
    if type == 0
      ...
      v = @original_item.nil? ? "None" : @original_item.tool_speed
  ...
  def draw_new_dndparam(dx, dy, type)
    ...
    if type == 0
      ...
      new_value = @temp_item.tool_speed
      v = @original_item.nil? ? 0 : @original_item.tool_speed
