...
module TSBS
  ...
  # --------------------------------------------------------------------------
  BusyPhases = [:intro, :skill, :prepare, :collapse, :forced, :return,:aim_target]
...
module TSBS_BattlerDB
  ...
  attr_accessor :aim_target_key  # Resize flag
  ...
  def load_tsbs
    ...
    @aim_target_key = TSBS::Aim_Target
...
class RPG::UsableItem < RPG::BaseItem
  ...
  attr_accessor :saving_throw_rolled
  # --------------------------------------------------------------------------
  # New method : load TSBS notetags
  # --------------------------------------------------------------------------
  def load_tsbs
...
class Game_Battler < Game_BattlerBase
  ...
  def update
    ...
    @shadow_point.update_move # Shadow repositiona
    if fiber_obj      
  ...
  def phase_sequence
    hash = {
      ...
      :aim_target => method(:aim_target_key),
    }
    return hash
  end
  # --------------------------------------------------------------------------
  ...
  def tsbs_main_fiber
    ...
    if flip_val == :ori || (flip_val.nil? && battle_phase != :skill)
      @flip = default_flip unless state?(271)
    end
    ...
    if area_flag && target_array
      ...
      target_array.uniq.each do |target|
        ...
        if !item.ignore_anim_guard? && item.parallel_anim?
          if enable_anim_guard?(item,target,anim_guard_state)
  ...
  def setup_cast
    ...
    self.animation_id = @acts[1].to_i || item_in_use.animation_id
  ...
  def setup_anim
    if $game_temp.one_animation_flag || (@acts[1].nil? && item_in_use &&
      ...
      mirror = flip || @acts[2]
      $game_temp.one_animation_id = anim_id.to_i
  ...
  def setup_icon
    return TSBS.error(@acts[0], 1, @used_sequence) if @acts.size < 2
    #puts "[Setup Icon]#{@acts[0]},#{@acts[1]},#{@acts[2]}"
    @icon_key = @acts[1]
  ...
  def anim_guard_id
    states.each do |state|
      if state && state.anim_guard > 0
        return state.anim_guard
      end
    end
    return 0
  end
  ...
  # --------------------------------------------------------------------------
  # New method : anim_guard_state
  # --------------------------------------------------------------------------
  def anim_guard_state
    states.each do |state|
      if state && state.anim_guard > 0
        return state
      end
...
class Game_Party < Game_Unit
  ...
  def max_battle_members
    #TSBS::ActorPos.size
    $game_variables[YEA::PARTY::MAX_MEMBERS_VARIABLE] = YEA::PARTY::MAX_BATTLE_MEMBERS if $game_variables[YEA::PARTY::MAX_MEMBERS_VARIABLE] == 0
...
class Sprite_AnimState < Sprite_Base
  ...
  def update_state_anim
    ...
    #battler.multi_animation_id.push(battler.state_anim)
...
class Sprite_Projectile < Sprite_Base
  ...
  def apply_item(trg, animation)
    ...
    guard_state = trg.anim_guard_state
      
    cond = trg.enable_anim_guard?(item,anim_id,guard_state)
  ...
  def start_projectile
    ...
    when PROJ_POSITION_NONE; ypos = xpos = 0
    when PROJ_POSITION_LAST_TARGET
      #p sprintf(subj.to_s)
      ypos = subj.last_target.y
      xpos = subj.last_target.x
    else; ypos = subj.y;
    end
...
class Scene_Battle < Scene_Base
  ...
  # --------------------------------------------------------------------------
  def use_item
    #puts "Main Target: #{$Battle_Current_Target.name}" if @subject.actor?
    item = @subject.current_action.item
    $current_using_item = @subject.current_action.item
    $current_user = @subject
    
    #-------------------------------------------------
    #   enemy use
    #
    if item.is_a?(RPG::Skill) && item.id == 760
      item = $data_items[$game_troop.item_require]
      message = sprintf("%s uses item: %s!",@subject.name,item.name)
    end
    
    #-------------------------------------------------
    #   Animate Dead
    #   tag: skill
    if item.is_a?(RPG::Skill) && item.id == 664
      for battler in $game_troop.members
        if battler.dead? && battler.is_minon?
          $force_target_enemy = battler
          battler.revive
          battler.purify_all
          battler.visible = false
          puts "Animated Alive: #{battler.alive?}"
          break
        end
    ...
    #-------------------------------------------------
    #   Enemy Clean up animated dead
    #
    if @subject.enemy? && item.for_opponent?
      for battler in $game_troop.alive_members
        next if @subject == battler
        $force_target = battler if battler.state?(271)
        animated_dead_target = battler
      end
    end
    ...
    if !$game_troop.item_target.nil? && @subject.enemy?
      targets = [$game_troop.item_target] 
      puts "[Enemy AI] Accurate target: #{targets[0].name}"
    end
    
    #-------------------------------
    # invert target bug fix
    #
    if $item_invert_target.size > 0
      for command in $item_invert_target
        if command[0] == item
          puts "[Item Using] Invert target -> #{command[1].name}"
          targets = [command[1]]
          $item_invert_target.delete(command)
          break
        end
      end
    end
    ...
    #----------------------------- Forced target
    if !$force_target.nil? && @subject.enemy?
      puts "Force target: #{$force_target}"
      targets = [$force_target]
    end
    #----------------------------- Forced target(target is enemy)
    if !$force_target_enemy.nil? && @subject.actor?
      puts "Force target: #{$force_target_enemy}"
      targets = [$force_target_enemy]
    end
    #------------------------------ Animated Enemy's target
    if @subject.state?(271) && @subject.enemy?
      puts "Animeted Dead Target: #{animated_dead_target.name}"
      targets = [animated_dead_target]
    end
    #------------------------------
    $game_troop.item_target = nil if @subject.enemy?
    
    show_action_sequences(targets, item, @subject)
    $force_target = nil if @subject.enemy?
    $force_target_enemy = nil if @subject.actor?
  end
  # --------------------------------------------------------------------------
  ...
  def check_collapse(target)
    ...
    if target.state?(target.death_state_id) || 
      ...
      battlers = $game_troop.members + $game_party.battle_members
      #-----------------------------------------------------------
      #   *) Death Syphon
      #-----------------------------------------------------------
      for battler in battlers
        if battler.state?(270) && !target.dead_drained && battler.opposite?(target)
          amount = [target.mp.to_i,battler.mmp * 0.2].min.to_i
          battler.mp += amount
          battler.damage.push([amount,"MP"])
          target.mp = 0
          battler.multi_animation_id.push(355)
          target.dead_drained = true
        end
  ...
  def tsbs_invoke_item(target, item, subj = @subject)
    return if item.nil? || item.is_missile
