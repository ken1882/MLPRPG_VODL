...
class Window_EquipStatus < Window_Base
  ...
  def get_param_name(id)
    return "無" if id < 0 || id >= DND::PARAM_NAME.size
  ...
  def draw_actor_attack_bonus(x,y)
    ...
    draw_text(0, y, w, line_height,"攻擊骰加成")
  ...
  def draw_actor_armor_class(x,y)
    w = contents.width
    change_color(system_color)
    draw_text(0, y, w, line_height,"護甲等級")
    change_color(normal_color)
  ...
  def draw_hoof_damage(x,y)
    ...
    if @actor.equips[0] && @actor.equips[0].damage_index.size < 1
      draw_text(x, y, w, line_height, "彈藥傷害")
    else
      draw_text(x, y, w, line_height, "武器傷害")
    end
    ...
    if @actor.equips[0].nil?
      ...
      for feat in index
        info = sprintf("%dd%d +(%d), %s; %s",feat[0],feat[1],feat[2], DND::ELEMENT_NAME[feat[3]], get_param_name(feat[4]))
        draw_text(x, (y + cnt * line_height), w, line_height, info , 2)
        cnt += 1
      end
    end
  ...
  def draw_item_type(x,y)
    w = contents.width
    change_color(system_color)
    draw_text(0, y, w, line_height,"類別")
    change_color(normal_color)
    if @temp_item.is_a?(RPG::Weapon)
  ...
  def draw_dndparam_compare(x,y,type, ori = false)
    ...
    change_color(system_color)
    if type == 0
      draw_text(0, y, contents.width, line_height,"攻擊骰加成")
    elsif type == 1
      draw_text(0, y, contents.width, line_height,"護甲等級")
    elsif type == 2
      draw_text(0, y, contents.width, line_height,"速度")
    elsif type == 3
      draw_text(0, y, contents.width, line_height,"距離")
    elsif type == 4
      draw_text(0, y, contents.width, line_height,"傷害")
    end
  ...
  def draw_current_dndparam(dx, dy, type, ori = false)
    ...
    if type == 0
      ...
      v = @original_item.nil? ? "無" : ( 20 - (@original_item.tool_cooldown / 5) )
      draw_text(0, dy, dw, line_height, v, 2)
    elsif type == 3
      v = @original_item.nil? ? "無" : @original_item.tool_distance
      draw_text(0, dy, dw, line_height, v, 2)
    elsif type == 4
  ...
  def draw_weapon_speed(x,y)
    w = contents.width
    change_color(system_color)
    draw_text(0, y, w, line_height, "速度")
    change_color(normal_color)
