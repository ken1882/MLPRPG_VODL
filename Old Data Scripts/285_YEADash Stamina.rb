...
module YES
  module DASH
    ...
    # - General Settings -
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    STAMINA_DEFAULT    = 100    # Default Stamina for player.
    STAMINA_PER_FRAME  = 1      # Default Stamina cost per frame while dashing.
    MOVE_RESTORE       = true  # Set this to false to disable restoring
    ...
    DISABLE_SWITCH     = 17      # Switch to toggle Stamina Feature.
...
module YES
  ...
  def self.set_max_stamina(amount = 10)
    $game_variables[YES::DASH::STAMINA_VARIABLE] += amount
  end
...
class Game_Player < Game_Character
  ...
  def update_stamina_recover
    ...
    return if process_moving && !YES::DASH::MOVE_RESTORE
...
class Window_Stamina < Window_Base
  ...
  #--------------------------------------------------------------------------
  def initialize
    super(1, Graphics.height-50 , YES::DASH::WINDOW_WIDTH, 50)
  ...
  # draw_stamina_bar
  #--------------------------------------------------------------------------
  def draw_stamina_bar(x, y, width = 80)
  ...
  #--------------------------------------------------------------------------
  def update
    if $game_system.hide_all_windows?
      self.hide
      return
    end
    if $game_player.dash? || @time > 0
      super
      update_input
      update_show
      refresh
  ...
  def update_show
    ...
    @time = 180
    self.x += 8 if self.x < 1
    @show = false if self.x >= 1
  end
  ...
  def update_input
    ...
    return unless $game_player.process_moving
