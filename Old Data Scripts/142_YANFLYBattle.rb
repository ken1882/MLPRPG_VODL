...
module YEA
  module BATTLE
    ...
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    ENABLE_POPUPS  = false     # Set this to false if you wish to disable them.
...
module Switch
  ...
  def self.hide_popups
    return false if Float(YEA::BATTLE::HIDE_POPUP_SWITCH) <= 0
...
class Window_BattleHelp < Window_Help
  ...
  def update_battler_name
    ...
    if @actor_window.active
      ...
      $Battle_Current_Target = battler
      $Battle_Original_Target = battler
