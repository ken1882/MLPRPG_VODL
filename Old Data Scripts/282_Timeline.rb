...
module GameTime
  ...
  def self.init
    ...
    $game_time_clock_visibility = false
  end
  ...
  def self.update
    ...
    case SceneManager::scene_is?(Scene_Map)
      when true
        if $game_message.visible == true && PAUSE_IN_MESSAGE then else
          $game_time += 1 if Graphics.frame_count % TIME_COUNT == 0 
        end
      when false
    ...
    if $game_time >= 1440 then
        $game_time %= 1440 
        $game_day += 1
    end
      
    $game_variables[TIME_VARIABLE] = $game_time if USE_VARIABLE
    $game_variables[DAY_VARIABLE] = $game_day if USE_VARIABLE
  ...
  def self.time?
    ...
    return "Day:" + $game_day.to_s + " | " +hour.to_s + ":" + minute.to_s + " " + meri
  end
  ...
  def self.tint(tint = 60)
    ...
    if SceneManager::scene_is?(Scene_Map) then else return end
      ...
      if !$game_switches[NIGHT_DAY_SWITCH] #if indoor
  ...
  class Window_Clock < Window_Base
    ...
    def initialize
      ...
      when 1
        super(0,0,205,56)
      when 2
        super(Graphics.width-205,0,205,56)
      when 3
        super(0,Graphics.height-56,205,56)
      when 4
    ...
    def update
      ...
      self.contents.clear
      self.contents.draw_text(0,0,205,24,GameTime::time?)
...
class Scene_Map < Scene_Base
  ...
  def create_all_windows
    use_clock = ($game_system.hide_all_windows? ^ USE_CLOCK)
